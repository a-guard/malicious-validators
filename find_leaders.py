...
def get_slot_leader(slot):
    ...
    for revenue in js['result']['rewards']:
        if revenue['rewardType'] == 'Fee':
            slot_and_leaders[slot] = revenue['pubkey']
            break


with open('data/filtered_sandwitches.json', 'r') as f: d = orjson.loads(f.read())

with ThreadPoolExecutor(max_workers=600) as pool:
    for _ in pool.map(get_slot_leader, list(d)): pass
